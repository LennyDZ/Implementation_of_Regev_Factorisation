...
def solve_samples_for_factors(
    ...
    *,
    ...
    for v in LB:
        ...
        if B in [R(-1), R(1)]:
            n_trivial_factor=n_trivial_factor+1
    ...
    return factors, n_factor, n_trivial_factor

def lattice_postprocessing(vectors, R, d, gamma, alpha, verbose=False):
    """
    Implements the lattice-based postprocessing algorithm according to Ragavan's procedure.

  
        @param vectors: List of Sage-compatible sampled vectors (w_i) in R^d/Z^d.
        @param N: The integer N (modulus in factoring).
        @param d: Dimension of the lattice (length of each vector).
        @param gamma: Positive real number for target size of B (gamma * d).
        @param alpha: Positive real number for subset size (alpha * d).
        @param verbose: Whether to print verbose status messages.

    Returns:
        A subset B' of vectors (w_i) that satisfy the conditions, ready for postprocessing.
    """
    from sage.all import matrix, block_matrix, QQ, ZZ
    ...
    m = len(vectors)
    B = set()
    gamma_d = ceil(gamma * d)
    alpha_gamma_d = ceil((alpha - gamma) * d)

    if verbose:
        print(f"Starting lattice postprocessing with {m} vectors, S={S}, d={d}, gamma={gamma}, alpha={alpha}.")

    # Main loop: Continue until |B| >= gamma * d
    while len(B) < gamma_d:
        if verbose:
            print(f"Current size of B: {len(B)}, target size: {gamma_d}.")

        # Step (a): Select subset E
        E = [i for i in range(m) if i not in B]
        if len(E) == 0:
            if verbose:
                print("No more vectors to select from. Exiting loop.")
            break

        if len(E) > alpha_gamma_d:
            E = E[:alpha_gamma_d]

        if verbose:
            print(f"Selected subset E (size {len(E)}): {E}")

        # Step (b): Define W
        W_vectors = [vectors[i].list() for i in E]
        W = matrix(QQ, W_vectors).transpose()

        if verbose:
            print(f"Matrix W shape: {W.nrows()} x {W.ncols()}")

        # Step (c): Define H
        Id_d = S * matrix.identity(d)
        SW = S * W
        Id_E = matrix.identity(len(E))
        Zero_block = matrix(ZZ, len(E), d)

        H = block_matrix([[Id_d, SW], [Zero_block, Id_E]])

        if verbose:
            print(f"Lattice basis matrix H shape: {H.nrows()} x {H.ncols()}")

        # Step (d): Apply LLL reduction
        H_reduced = H.LLL()

        if verbose:
            print("LLL reduction complete.")

        # Step (e): Extract shortest vector
        shortest_vector = H_reduced.row(0)
        beta = shortest_vector[:d]
        a_i = shortest_vector[d:]

        if verbose:
            print(f"Shortest vector (beta, a_i): {beta}, {a_i}")

        # Step (f): Update B
        for idx, a in zip(E, a_i):
            if a != 0:
                B.add(idx)

        if verbose:
            print(f"Updated size of B: {len(B)}")

    # Select B'
    B_prime = list(B)[:gamma_d]
    selected_vectors = [vectors[i] for i in B_prime]

    if verbose:
        print(f"Postprocessing complete. Selected {len(selected_vectors)} vectors for further analysis.")
