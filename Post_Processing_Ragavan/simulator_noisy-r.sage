...
class Simulator_noisy_r:
    ...
    def _sample_noisy_vector(self, D, R):
        """
        Samples a vector close to a random vector in L^* / Z^d.
    
        Args:
            D: The parameter D that specifies the discretization.
            R: The parameter R that specifies the standard deviation.
    
        Returns:
            A d-dimensional list with rational entries close to L^* / Z^d.
        """
        from sage.all import QQ
    
        # Sample a random vector in L^* / Z^d
        t = [QQ(IntegerModRing(D).random_element().lift(), D) for _ in range(self.d)]
        
        # Compute the closest vector to t in the dual lattice
        ...
        # Generate the sample
        sampled_vector = sample_distribution()  # Output of the Gaussian sampler
        
        # Convert the sample from the Gaussian distribution to rational entries
        result = [QQ(int(x), D) for x in sampled_vector]
    ...
    def _sample_bad_vector(self, D):
        """
        Samples a bad vector by selecting uniformly random components from {0, 1/D, ..., (D-1)/D}.
    
        Args:
            D: The parameter D that specifies the discretization.
    
        Returns:
            A d-dimensional list with rational entries sampled as {0, 1/D, ..., (D-1)/D}.
        """
        from sage.all import QQ
    
        # Sample each component uniformly at random
        bad_vector = [QQ(IntegerModRing(D).random_element().lift(), D) for _ in range(self.d)]
    ...
    import random as py_random  # Importer explicitement le module random de Python

    def sample_error_model(self, R, m1, p, eta=1, verbose=False):
        """
        Samples vectors with the error model specified in the image.
    
        This function simulates both uncorrupted and corrupted samples using the error model:
        - With probability (1-p), the sample is uncorrupted.
        - With probability p, the sample is corrupted either additively or by replacement based on eta.
    
        @param R  The parameter R specifying the standard deviation for the Gaussian distribution.
        @param m1 The number of vectors to sample.
        @param p  The probability of corruption for each sample.
        @param eta Specifies the type of error: 1 for additive errors, 0 for replacement errors.
        @param verbose A flag to enable verbose status messages.
    
        @return A list of m1 vectors sampled with the specified error model.
        """
        # Setup and start a timer.
        timer = Timer().start()
        ...
        if verbose:
            print(f"Sampling {m1} vectors with error model (p={p}, eta={eta})...")
    
        samples = []
    
        for _ in range(m1):
            # Sample a "good" vector from the Gaussian distribution
            v = self._sample_noisy_vector(D, R)
    
            # Determine whether to corrupt the sample
            if py_random.random() < p:  # Utiliser py_random pour éviter les conflits
                # Sample error term from noise distribution D
                epsilon = self._sample_bad_vector(D)
    
                if eta == 1:
                    # Additive error
                    corrupted_sample = [vi + ei for vi, ei in zip(v, epsilon)]
                elif eta == 0:
                    # Replacement error
                    corrupted_sample = epsilon
            else:
                # Uncorrupted sample
                corrupted_sample = v
    
            # Add the (potentially corrupted) sample to the list
            samples.append(corrupted_sample)
    
        # Shuffle samples to randomize the order of corrupted and uncorrupted samples
        py_random.shuffle(samples)  # Utiliser py_random.shuffle pour éviter les conflits
